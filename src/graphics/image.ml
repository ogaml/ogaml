...
let mirror_data img = 
  ...
  for x = 0 to img.width - 1 do
    for y = 0 to img.height - 1 do
      let c = get img Vector2i.({x; y}) |> Result.assert_ok in
      set img_rev Vector2i.({x; y = img.height - 1 - y}) (`RGB c) |> Result.assert_ok;
    done
  done;
  img_rev
...
let mipmap img lvl = 
  ...
  for x = 0 to new_width - 1 do
    for y = 0 to new_height - 1 do
      let c = get img {Vector2i.x = x lsl lvl; y = y lsl lvl} |> Result.assert_ok in
      set new_img {Vector2i.x; y} (`RGB c) |> Result.assert_ok
    done;
  done;
  new_img
