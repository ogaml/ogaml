...
let normal, color, position = 
  ...
  MyVertex.attribute "normal"   AttributeType.vector3i |> assert_ok,
  MyVertex.attribute "color"    AttributeType.int |> assert_ok,
  MyVertex.attribute "position" AttributeType.vector3f |> assert_ok
...
let make_vertex nm pos (r,g,b) =
  let v = MyVertex.create () |> assert_ok in
